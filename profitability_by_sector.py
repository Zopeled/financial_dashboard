...
api_key = 'b4ec5eef495822971cbf5b88277edbf2'
...
for sector in sectors:
    ...
    screener = requests.get(
        f'https://financialmodelingprep.com/api/v3/stock-screener?{marketcapmorethan}=1000000000&volumeMoreThan=10000&sector={sector}&exchange={exchange}&limit={number_of_companies}&apikey={api_key}').json()
    ...
    for company in symbols:
        try:
            n = n + 1
            IS = requests.get(
                f'https://financialmodelingprep.com/api/v3/income-statement/{company}?period=quarter&apikey={api_key}').json()
